/* =====================================================================
 * 개인화 마우스 보정 실험 v2 — app.js
 * UI 인식(DOM) · 시선추적(MediaPipe Face Landmarker) · 개인 클릭 패턴 학습의 기여도 분리
 * 실험설계서(2026 기초자율연구) 구현.
 *
 * v1(상위 experiment/) 대비 차이는 이 파일 안의 Gaze 모듈뿐이다.
 * WebGazer(눈 주변 픽셀을 그대로 회귀에 넣는 방식) 대신, MediaPipe Face Landmarker로
 * 뽑은 기하학적 특징(눈동자가 눈 안에서 치우친 비율, 코끝 위치, 눈 간격 등 =
 * 머리 위치·거리 정보가 특징에 명시적으로 들어감)에 자체 릿지 회귀를 학습시킨다.
 * 그 외 시행 로직·보정 로직·분석·내보내기는 v1과 동일 — Gaze.dominantZone()이
 * 반환하는 {zone, n, confidence} 형식만 맞으면 나머지 코드는 엔진이 뭐든 상관없다.
 *
...
// ES 모듈은 항상 strict mode라 별도 'use strict' 불필요.
...
const App = {
  ...
  cfg: {
    ...
    nDistractors: 5,
    gazeSmoothAlpha: 0.45, // 시선 좌표 지수이동평균 계수(낮을수록 부드럽지만 반응 지연↑). 프레임별 예측이 원래 노이즈가 커서 무보정 시 점이 튐.
  },
  gazeReady: false,       // 시선추적(MediaPipe) 로드+보정 완료 여부
...
/* localStorage 복원 */
try {
  const saved = JSON.parse(localStorage.getItem('mc_trials_v2') || '[]');
...
/* ------------------------------------------------------------------ */
/* 3. 시선추적 (MediaPipe Face Landmarker 기반, 자체 릿지 회귀)          */
/*    WebGazer처럼 눈 주변 픽셀을 통째로 회귀에 넣는 대신, 랜드마크에서   */
/*    기하학적 특징(눈동자-눈 상대위치, 코끝 위치, 눈 간격=거리 근사,     */
/*    좌우 눈 폭 비율=고개 회전 근사)을 계산해 그 특징으로 회귀를 학습.   */
/*    머리 위치·거리 정보가 특징에 명시적으로 들어가므로 WebGazer보다     */
/*    자세 변화에 원리적으로 더 강건할 것으로 기대(=이 프로젝트 v2의 가설).*/
/* ------------------------------------------------------------------ */

/* --- 3a. 선형대수: 릿지 회귀 (특징 정규화 + 가우스-조던 소거) ----------- */
...
function fitRidge(X, y, lambda){
  ...
  for (let a=0;a<d;a++) XtX[a][a] += lambda;   // bias(마지막 항)에는 정규화 안 걸어도 되지만 소표본이라 걸어도 무방
  return solveLinearSystem(XtX, Xty);
}
function predictLin(w, x){ let s=0; for (let i=0;i<w.length;i++) s+=w[i]*x[i]; return s; }

/* --- 3b. MediaPipe 랜드마크 → 특징 벡터 -------------------------------- */
// 표준 MediaPipe FaceMesh(468) + iris(468~477) 인덱스.
const LM = {
  L_OUT:33, L_IN:133, L_UP:159, L_DN:145, L_IRIS:468,   // 눈 A(카메라 기준 왼쪽)
  R_IN:362, R_OUT:263, R_UP:386, R_DN:374, R_IRIS:473,  // 눈 B(카메라 기준 오른쪽)
  NOSE:1,
};
function eyeFeat(lm, outer, inner, up, dn, iris){
  const o=lm[outer], i=lm[inner], u=lm[up], d=lm[dn], ir=lm[iris];
  const w = Math.hypot(i.x-o.x, i.y-o.y) || 1e-6;
  const h = Math.hypot(d.x-u.x, d.y-u.y) || 1e-6;
  const midX=(o.x+i.x)/2, midY=(u.y+d.y)/2;
  return { dx:(ir.x-midX)/(w/2), dy:(ir.y-midY)/(h/2), w };
}
/* 랜드마크 배열(468~477, 정규화 0..1) → 9차원 특징(+상수항은 학습 시 별도 추가) */
function featuresFromLandmarks(lm){
  const eA = eyeFeat(lm, LM.L_OUT, LM.L_IN, LM.L_UP, LM.L_DN, LM.L_IRIS);
  const eB = eyeFeat(lm, LM.R_IN, LM.R_OUT, LM.R_UP, LM.R_DN, LM.R_IRIS);
  const nose = lm[LM.NOSE];
  const interocular = Math.hypot(lm[LM.L_IRIS].x-lm[LM.R_IRIS].x, lm[LM.L_IRIS].y-lm[LM.R_IRIS].y) || 1e-6;
  const asym = eA.w/eB.w;   // 고개를 좌우로 돌리면 한쪽 눈 폭이 원근 때문에 짧아짐 → 회전 근사치
  return [eA.dx, eA.dy, eB.dx, eB.dy, nose.x, nose.y, interocular, asym];
}

const Gaze = {
  video:null, canvas:null, ctx:null,
  landmarker:null,
  lastFeat:null,       // 가장 최근 프레임의 특징 벡터(보정 클릭 때 이걸 표본으로 씀)
  calibSamples:[],      // {feat, x, y}
  model:null,           // {wx, wy, mean, std}
  running:false,
  showDebug:false,      // 보정 화면에서만 웹캠 미리보기에 랜드마크 점을 그림
  ...
  async start(){
    ...
    try {
      ...
      this.canvas = document.getElementById('camOverlay');
      ...
      await new Promise((res,rej)=>{
        ...
        setTimeout(()=>rej(new Error('웹캠 스트림 응답 없음')), 10000);
      });
      ...
      try {
        this.landmarker = await FaceLandmarker.createFromOptions(vision, {
          baseOptions: { modelAssetPath, delegate:'GPU' }, runningMode:'VIDEO', numFaces:1,
        });
      } catch(gpuErr){
        // 일부 환경(GPU 미지원/드라이버 문제)에서 GPU 위임이 실패할 수 있어 CPU로 재시도.
        console.warn('GPU 위임 실패, CPU로 재시도:', gpuErr);
        this.landmarker = await FaceLandmarker.createFromOptions(vision, {
          baseOptions: { modelAssetPath, delegate:'CPU' }, runningMode:'VIDEO', numFaces:1,
        });
      }
      ...
      console.warn('MediaPipe 시작 실패:', e);
  ...
  _handle(result, t){
    ...
    if (!this.model) return;   // 보정 전이면 예측 안 함(보정 화면에서는 클릭 시 표본만 채집)
    const xn = feat.map((v,i)=> (v-this.model.mean[i])/this.model.std[i]).concat([1]);
    const px = predictLin(this.model.wx, xn);
    const py = predictLin(this.model.wy, xn);

    // v1과 동일한 지수이동평균 스무딩 재사용.
    const a = App.cfg.gazeSmoothAlpha;
    if (!App.gazeSmooth) App.gazeSmooth = { x:px, y:py };
    else { App.gazeSmooth.x += (px-App.gazeSmooth.x)*a; App.gazeSmooth.y += (py-App.gazeSmooth.y)*a; }
  ...
  /* 진단용: 보정 화면에서만 웹캠 미리보기 위에 눈동자·눈 코너·코끝 표시 */
  _drawDebug(lm){
    const c = this.ctx, W = this.canvas.width, H = this.canvas.height;
    c.clearRect(0,0,W,H);
    if (!lm || !this.showDebug) return;
    const pts = [LM.L_OUT,LM.L_IN,LM.L_UP,LM.L_DN,LM.L_IRIS,LM.R_IN,LM.R_OUT,LM.R_UP,LM.R_DN,LM.R_IRIS,LM.NOSE];
    c.fillStyle = '#5ad1c9';
    for (const idx of pts){
      const p = lm[idx];
      c.beginPath(); c.arc(p.x*W, p.y*H, 1.6, 0, Math.PI*2); c.fill();
    }
  },

  /* 릿지 회귀 학습(보정 종료 시 호출). 표본 부족하면 false. */
  trainModel(){
    const samples = this.calibSamples;
    if (samples.length < 15){ App.gazeError = '보정 표본 부족(9점 중 최소 3점은 5클릭씩 채워야 함, 얼굴 인식 안 된 클릭은 표본에서 제외됨)'; return false; }
    const d = samples[0].feat.length;
    const mean = new Array(d).fill(0), std = new Array(d).fill(0);
    for (const s of samples) for (let i=0;i<d;i++) mean[i] += s.feat[i];
    for (let i=0;i<d;i++) mean[i] /= samples.length;
    for (const s of samples) for (let i=0;i<d;i++) std[i] += (s.feat[i]-mean[i])**2;
    for (let i=0;i<d;i++) std[i] = Math.sqrt(std[i]/samples.length) || 1;
    const Xn = samples.map(s => s.feat.map((v,i)=> (v-mean[i])/std[i]).concat([1]));
    const lambda = 3.0;
    const wx = fitRidge(Xn, samples.map(s=>s.x), lambda);
    const wy = fitRidge(Xn, samples.map(s=>s.y), lambda);
    this.model = { wx, wy, mean, std };
    return true;
  },

  /* 클릭 직전 buffer 구간의 시선 표본들. (v1과 동일) */
  recentSamples(clickT, windowMs){
    const lo = clickT - windowMs;
    return App.gazeBuffer.filter(s => s.t >= lo && s.t <= clickT + 5);
  },
  /* 최근 시선 표본들의 다수결 구역. 없으면 null. (v1과 동일) */
  ...
  resume(){ if (!this.running && this.landmarker){ this.running = true; this._loop(); } },
};
...
/* 시선 보정(Calibration) — 9점 각 5클릭. 각 클릭마다 그 순간의 MediaPipe 특징 벡터를
   해당 점의 화면 좌표와 짝지어 표본으로 모으고, 끝나면 릿지 회귀를 학습한다. */
async function startCalibration(){
  const ok = App.gazeReady || await Gaze.start();
  if (!ok){ alert('시선추적을 시작할 수 없습니다.\n\n원인: '+(App.gazeError||'알 수 없음')+'\n\n웹캠 권한/인터넷을 확인하세요. 시선 미사용으로도 C0·C1 실험은 가능합니다.'); go('menu'); return; }
  Gaze.calibSamples = [];
  Gaze.model = null;   // 재보정 시 이전 모델을 지우고 새로 학습
  Gaze.showDebug = true;   // 보정 중에는 웹캠 미리보기에 랜드마크 점을 그려 추적 상태를 육안 확인
  ...
  const dots=pts.map((p,idx)=>{
    ...
    d.addEventListener('click', ()=>{
      if (!Gaze.lastFeat){
        // 그 순간 얼굴 인식이 안 되면 표본으로 안 쓰고, 카메라를 보라고 시각적으로만 알려준다.
        d.style.outline='2px solid #ff6b6b'; setTimeout(()=> d.style.outline='', 250);
        return;
      }
      Gaze.calibSamples.push({ feat:Gaze.lastFeat.slice(), x:p.x, y:p.y });
  ...
  function endCalib(){
    ...
    Gaze.showDebug = false;
    const trained = Gaze.trainModel();
    go('menu'); updateStatusbar();
    if (trained) alert('보정 완료('+Gaze.calibSamples.length+'표본으로 회귀 학습). 이제 예비실험 A 또는 본실험을 진행할 수 있습니다.');
    else alert('보정 표본이 부족해 학습에 실패했습니다: '+(App.gazeError||'')+'\n다시 보정해 주세요(카메라를 보면서 클릭).');
  }
...
/* 9. 저장 / 내보내기                                                   */
/* ------------------------------------------------------------------ */
function persist(){ try{ localStorage.setItem('mc_trials_v2', JSON.stringify(App.trials)); }catch(e){} updateDataCount(); }
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  // 시선 사용 여부만 저장. 실제 웹캠 권한 프롬프트·모델 로딩은 보정 화면 진입 시(Gaze.start).
