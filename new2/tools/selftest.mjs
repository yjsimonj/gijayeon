 ...
 *
 * 검사 항목
 *   1) 시퀀스: 600회 = 4방향 × 150, 100회 블록마다 25씩, 학습 400 / 평가 200
 *      어느 쪽으로 잘라도 방향 균형 (계획서 §4.2, §6)
...
ok([...countByDirection(main).values()].every((v) => v === I.COUNTS.main / 4),
  `4방향 각 ${I.COUNTS.main / 4}회`, JSON.stringify([...countByDirection(main)]));
...
ok(vt.shifted, '세로 시행: 시작 버튼이 밀림 (중앙 + 450px는 화면을 벗어난다)');
