...
이 범위가 넓은 것은 실제로 넓기 때문이다. 계획서 설계(8명 × 평가 200회 = 1600시행,
McNemar + Bonferroni, α=.05)로 "C > A 그리고 C > B" 에 도달할 검정력을 시뮬레이션한
결과 — 공통편향 (0.4, −1.3)px, 개인편향 축별 SD 1.6px, 클릭잡음 σ:

| 버튼 | σ=4.0 성공률 / 검정력 | σ=5.5 성공률 / 검정력 |
|---|---|---|
| 10px | 52% / 96% | 33% / **40%** |
| 12px | 65% / 97% | 44% / 61% |
| 16px | 85% / 96% | 64% / 77% |
...
**작은 버튼이 항상 유리한 게 아니다.** 클릭잡음이 test01 수준(σ=5.5)이면 10px에서
검정력이 40%까지 떨어진다 — 버튼이 오차보다 작으면 보정을 해도 어차피 빗나가기
때문이다. 20px는 두 σ 어디서도 무너지지 않는다. 확실히 나쁜 것은 24px 이상이다.

> 이전 판의 표는 "성공률 80% 이상이면 크기를 낮춰라" 였다. σ가 클 때 그 지시를
> 따르면 검정력이 오히려 떨어지므로 위 시뮬레이션 결과로 교체했다.
...
```powershell
cd c:\Lab\gijayoun\new2
node tools/selftest.mjs          # 69개 — 시퀀스·기하·스키마·CSS 정합
